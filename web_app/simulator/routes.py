...
def account():
    ...
    if form1.validate_on_submit():
        ...
        user_transactions=Transactions(username=current_user.username,type='deposit',amount=form2.amount.data,date_placed=datetime.now(IST))
...
def withdraw():
    ...
    if form2.validate_on_submit():
        ...
        user_transactions=Transactions(username=current_user.username,type='withdraw',amount=form2.amount.data,date_placed=datetime.now(IST))
        db.session.add(user_transactions)
        db.session.commit()
    ...
    if form.validate_on_submit():
        ...
        if balance > form.stake.data:
            new_bet=Bets_placed(team_a=form.teamA.data,team_b=form.teamB.data,condition=form.bet_condtion.data,stake=form.stake.data,
                                ratio=form.ratio.data,return_=return_,username=current_user.username,date_placed=datetime.now(IST))
